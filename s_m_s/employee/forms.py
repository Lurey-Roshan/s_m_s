	...
	class Meta:
		...
		def __init__(self, *args, **kwargs):
			...
			if 'upazilla' in self.data:
				...
				self.fields['upazila'].queryset=self.instance.district.upazilla_set.order_by('name')
...
class JobInfoForm(forms.ModelForm):
	class Meta:
		...
		fields='__all__'
		widgets={
			'caregory':forms.Select(attrs={'class':'form-control'}),
			'joining_date':form.TextInput(attrs={'class':'form-control'}),
			...
			'recreation_leave_sue_year':forms.NumberInput(attrs={'class':'form-control'}),
			'expected_retirement_date':forms.NumberInput(attrs={'class':'form-control'})
		}
